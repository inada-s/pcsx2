...
bool TcpClient::Connect(const char *host, int port) {
...
#endif
                        printf("Connect fail 4 %d\n", error);
                        return false;
                    }

                    if (FD_ISSET(new_sock, &setE)) {
                        printf("Connect fail 5 %d\n", get_last_error());
                        return false;
                    }
...

    {
        ...
        if (getsockname(new_sock, reinterpret_cast<sockaddr *>(&name), &namelen) != 0) {
            ...
            local_ip_ = std::string(inet_ntoa(name.sin_addr));
        }
    }

    printf("TCP Connect: %s:%d ok\n", host, port);
    return true;
}
...
int TcpClient::Recv(char *buf, int len) {
    ...
    if (n < 0 && get_last_error() != L_EAGAIN && get_last_error() != L_EWOULDBLOCK) {
        printf("TCP Recv failed. errno=%d\n", get_last_error());
        this->Close();
    }
    if (n < 0) return 0;
    return n;
...
bool UdpRemote::Open(const char *host, int port) {
    ...
    if (host_entry == nullptr) {
        printf("UDP Remote::Initialize failed. gethostbyname %s\n", host);
        return false;
    }
...
bool UdpClient::Bind(int port) {
    ...
    if (::bind(new_sock, (struct sockaddr *) &recv_addr, sizeof(recv_addr)) < 0) {
        printf("gdxsv: bind() failed. errno=%d\n", get_last_error());
    ...
    if (::getsockname(new_sock, (struct sockaddr *) &recv_addr, &addr_len) < 0) {
        printf("gdxsv: getsockname() failed. errno=%d\n", get_last_error());
        closesocket(new_sock);
        return false;
    }
    ...
    printf("UDP Initialize ok: %s:%d\n", bind_ip_.c_str(), bind_port_);
    return true;
}
...
int UdpClient::RecvFrom(char *buf, int len, std::string &sender) {
    ...
    if (n < 0 && get_last_error() != L_EAGAIN && get_last_error() != L_EWOULDBLOCK) {
        printf("UDP Recv failed. errno=%d\n", get_last_error());
        return 0;
    }
